    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        runnable = new Runnable() {
            ...
            public void run() {
               ...
               //checks ir there is still some time left in the progressbar
                if(currentTime > 0 ){
                    handler.postDelayed(runnable, 100);
                } else {
                    //checks if the color of the arrow and button are the same
                    if(buttonState == arrowState){
                        //increase points and show them
                        currentPoints = currentPoints + 1;
                        tv_points.setText("Points: " + currentPoints);

                        //makes the speed higher after every turn(if the speed is 1 sec it makes it 2 sec)
